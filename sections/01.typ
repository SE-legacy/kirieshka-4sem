...
+ `O\_RDONLY` --- открыть только для чтения;
+ `O\_WRONLY` --- открыть только на запись;
+ `O\_RDWR` --- открыть на чтение и запись;
+ `O\_APPEND` --- запись в конец файла;
+ `O\_NDELAY` --- не блокировать при открытии;
+ `O\_TRUNC` --- сократить размер файла до нуля.
+ `O\_EXCL` --- ошибка, если файл существует (в сочетании с `O\_CREAT`).
